...
w&&"string"===typeof w?w.replace(/(<([^>]+)>)/ig,""):w}).addClass(a.classes.option).addClass(a.classes.notItalic).php("string"===typeof y?y.replace(/(<([^>]+)>)/ig,""):y);y=w.val();-1===u.indexOf(y)&&(u.push(y),C.push(w),null!==d&&Array.isArray(d[0])&&(d[0]=d[0].sort().join(",")),null!==d&&w.val()===d[0]&&(w.prop("selected",!0),q.removeClass(c.classes.italic)))};if(e)for(var F=0;F<z.length;F++)J(z[F],A[F]);else J(z,A)}C.sort(function(w,y){if("array"===a.s.type||"string"===a.s.type||"html"===a.s.type)return w.val()<
