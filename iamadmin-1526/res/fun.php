...
$a=1;
while ($a <= 4 && $sponser_id!=0) {
  $level = [80,60,40,20];
  $amt = $level[$a-1];
  mysqli_query($conn, "UPDATE `agent_income` SET `wallet`=`wallet`+$amt WHERE `agent_id`='$sponser_id'");
  mysqli_query($conn, "INSERT INTO `wallet_history`(`agent_id`, `amt`, `desp`, `status`)VALUES ('$sponser_id','$amt','Congratulation! You have got $amt from your $a level income.','0')");
