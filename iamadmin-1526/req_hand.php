...
if (isset($_REQUEST['cut_add_btn'])) {

  $agent_id = 1234;
  $r_id = mysqli_real_escape_string($conn, $_REQUEST['recive_id']);
  $amt = mysqli_real_escape_string($conn, $_REQUEST['amount']);
  $opt = mysqli_real_escape_string($conn, $_REQUEST['options']);


  $amount = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `admin` WHERE `admin_id`='$agent_id'"));
  $agentBal = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `agent_income` WHERE `agent_id`='$r_id'"));
  ...
  if (check_valid_user_id($r_id)) {
    if ($opt == "cut") {
      if ($agentBal['wallet'] >= $amt) {
        $sql = "UPDATE `agent_income` SET `wallet`=`wallet`-$amt WHERE `agent_id`='$r_id'";
        $sql2 = "UPDATE `admin` SET `balance`=`balance`+$amt WHERE `admin_id`='$agent_id'";
        $sql3 = "INSERT INTO `wallet_history`(`agent_id`, `amt`, `desp`, `status`) VALUES ('$r_id','$amt','Funds Debited', '1')";
        if ($conn->query($sql) === TRUE && $conn->query($sql2) === TRUE && $conn->query($sql3) === TRUE) {
          $_SESSION['w_pass'] = 1;
          header("Location: add_fund_cut_fund.php");
        } else {
          $_SESSION['w_pass'] = 2;
        }
      } else {
      ...
      if ($amt >= $amount['balance']) {
        $_SESSION['w_pass'] = 8;
      } else {

        $sql = "UPDATE `agent_income` SET `wallet`=`wallet`+$amt WHERE `agent_id`='$r_id'";
        $sql2 = "UPDATE `admin` SET `balance`=`balance`-$amt WHERE `admin_id`='$agent_id'";
        $sql3 = "INSERT INTO `wallet_history`(`agent_id`, `amt`, `desp`, `status`) VALUES ('$r_id','$amt','Funds Credited', '0')";
        if ($conn->query($sql) === TRUE && $conn->query($sql2) === TRUE && $conn->query($sql3) === TRUE) {
          $_SESSION['w_pass'] = 22;
          header("Location: add_fund_cut_fund.php");
        } else {
...
if (isset($_REQUEST['login_btn'])) {
  ...
  $data = mysqli_query($conn, "SELECT * FROM `admin` WHERE `admin_id`='$agent_id'");
