...
if (isset($_REQUEST['send_otp_btn'])) {
  ...
  if (check_for_mobile($num)) {
    ...
    //enter otp in db
    $sql = "INSERT INTO `mobile_numbers`(`agent_id`, `mobile_number`, `verification_code`, `verified`) VALUES ('$agent_id', '$num', '$otp', '0')";
...
if (isset($_REQUEST['verify_otp_btn'])) {
  ...
  $data = mysqli_query($conn, "SELECT * FROM `mobile_numbers` WHERE `mobile_number`='$num' AND `verified`='0'");

  $otp_info = mysqli_fetch_array($data);
  $otp = $otp_info['verification_code'];
  ...
  if ($uotp == $otp) {
    ...
    $agent_id = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `agent` WHERE agent_mob = '$num'"));

    $_SESSION['getId'] = $agent_id['agent_id'];
        ...
        <div class="card-body">
          ...
          <form action="forget-pass.php" method="post">
            <div class="input-group mb-3">
              <input type="text" name="user_mobile" class="form-control" value="<?php if ($_SESSION['mobile'] != ""){echo $_SESSION['mobile'];} else{ echo "";} ?>" placeholder="Mobile Number" pattern="[6-9]{1}[0-9]{9}" required>
